...
- [KOL-C-Web][04.08 → 19.08 **BÜYÜK ÖLÇÜDE ÇÖZÜLDÜ**] Yasal metinlerdeki **işletme kimlik bilgileri** verilmedi. | Etkilenen: `website/app/kvkk/page.tsx`, `website/app/mesafeli-satis/page.tsx`, `website/app/gizlilik/page.tsx` | **Durum:** sözleşmeye `SiteContent.legal` bloğu eklendi (`docs/openapi.yaml`), panelde "Yasal kimlik" bölümü açıldı ve değerler girildi: ticari unvan, işletme türü, merkez adresi, vergi dairesi/no. Kaynak 2024 vergi levhası (GİB onay kodu WVVA0KN3E20) — BLD, BBD ile aynı gerçek kişi işletmesi altında. MERSİS ve KEP **kalıcı olarak yok** (ticaret siciline kayıtlı tüzel kişi değil); bu satırlar artık "eksik" sayılmıyor, hiç gösterilmiyor. | **20.08 TAMAMLANDI:** telefon (BBD ile aynı hat) ve e-posta (`huseiyn.bardakci@benimlezzetdunyam.com.tr`) işletme sahibinden alındı, tohuma yazıldı ve canlı `contact` satırına da girildi — yasal sayfalarda artık gerçek değer görünüyor, "girilmesi gerekiyor" işareti kalmadı. `contact.address` bilinçli olarak hâlâ boş: buradaki adres müşteriye gösterilecek ziyaret adresidir ve catering mutfağının yeri teyit edilmedi; yasal metinlerin istediği merkez adresi `legal.registered_address`ten geliyor ve dolu.
- [KOL-C-Web][19.08] Gizlilik metninde **tek** açık madde kaldı: **ödeme hizmeti sağlayıcısının ticari adı**. Kodda gerçek sanal POS yok — `veykemtu/payment` altında nakit, havale ve SİMÜLASYON geçidi var (bkz. `docs/03` §"Faz 1 notu"). | Etkilenen: `website/components/site/legal-identity.tsx` `openLegalItems()` | Geçici çözüm: madde "yayın öncesi tamamlanacak" listesinde; panelde sağlayıcı adı girildiği anda listeden düşüyor ve kutu tamamen kayboluyor.
- [KOL-C-Web][19.08 **ÇÖZÜLDÜ**] Barındırma sağlayıcısı/konumu ve saklama süreleri. | **Sonuç:** sağlayıcı **Hetzner Online GmbH**, sunucu **Finlandiya** (`docs/08` §1 `62.238.102.197`; IP whois netname `CLOUD-HEL1`). Süreler işletme sahibinden alındı: sunucu erişim kayıtları **15 gün**, sipariş/fatura kayıtları **10 yıl**. Üçü de gizlilik ve KVKK metinlerine yazıldı; açık madde listesinden çıkarıldı — bunlar panelden düzenlenecek iş verisi değil, sunucu taşınırsa zaten kod değişikliği olur.
- [KOL-C-Web][19.08] **Yurt dışına aktarımın hukuki sebebi belirsiz.** Sunucu Finlandiya'da olduğu için kişisel veriler fiilen yurt dışında tutuluyor ve KVKK md.9 bunun açıkça bildirilmesini istiyor. | Etkilenen: `website/app/kvkk/page.tsx` §5 | Geçici çözüm: metne yalnızca **olgu** yazıldı ("veriler Finlandiya'daki sunucularda tutulmaktadır"); aktarımın hangi sebebe dayandığı (açık rıza, yeterlilik kararı, standart sözleşme) **iddia edilmedi** — bu hukuk danışmanının belirleyeceği bir konu.
