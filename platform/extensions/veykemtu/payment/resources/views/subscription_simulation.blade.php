<!doctype html>
{{-- Abonelik dönem ödemesinin simülasyon sayfası. Sipariş ödemesinden AYRI: --}}
{{-- burada sipariş yok, ödenen şey 30 günlük bir sözleşme dilimi. --}}
<html lang="tr">
<head>
<meta charset="utf-8">
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .dokum { color:var(--nötr); font-size:13px; margin:0 0 20px; padding:0 4px; }
...
<div class="kutu">
  ...
  <div class="uyari">
    ...
    hiçbir yere kaydedilmez. Sanal POS entegrasyonu tamamlanana kadar geçerlidir.
  </div>

  <div class="kart">
    <h1>Abonelik dönem ödemesi</h1>
    <p class="alt">Abonelik A-{{ $payment->subscription_id }} · {{ $period }}</p>

    <div class="tutar">
      <span>Ödenecek tutar</span>
      <b>{{ $total }} ₺</b>
    </div>
    {{-- Çarpanlar ekranda: "neden bu tutar" sorusu ödeme sayfasında --}}
    {{-- cevaplanmazsa müşteri ödemeyi yarıda bırakıp telefona sarılıyor. --}}
    <p class="dokum">
      {{ $payment->portions_planned }} porsiyon ×
      {{ number_format($payment->unit_price_kurus / 100, 2, ',', '.') }} ₺
    </p>

    @if ($alreadySettled)
      <div class="hata">Bu dönem ödemesi zaten sonuçlanmış. Tekrar ödeme alınmaz.</div>
      <a href="{{ $returnUrl }}"><button type="button">Aboneliğe dön</button></a>
    @else
      ...
      <form method="post" action="{{ url('/abonelik-odeme-simulasyon/'.$hash) }}?return={{ urlencode($returnUrl) }}">
        @csrf
