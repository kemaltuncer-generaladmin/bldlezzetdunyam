...
{{-- Kuveyt Türk entegrasyonu geldiğinde bu sayfa devre dışı kalır. --}}
<html lang="tr">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="robots" content="noindex, nofollow">
<title>Ödeme — Benim Lezzet Dünyam</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .tutar { display:flex; justify-content:space-between; align-items:baseline;
           padding:14px 16px; background:var(--zemin); border-radius:12px; margin-bottom:20px; }
...
<div class="kutu">
  ...
  <div class="kart">
    <h1>Ödeme</h1>
    <p class="alt">Sipariş S-{{ $order->order_id }}</p>
    ...
    @if ($alreadyPaid)
      <div class="hata">Bu sipariş zaten ödenmiş. Tekrar ödeme alınmaz.</div>
      <a href="{{ $returnUrl }}"><button type="button">Siparişe dön</button></a>
    @else
      ...
      <form method="post" action="{{ url('/odeme-simulasyon/'.$hash) }}?return={{ urlencode($returnUrl) }}">
        @csrf
